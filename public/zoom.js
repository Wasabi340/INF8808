...
var graphs = {}; // TODO: This variable represents a list of all graphs curently present on the site
//
...
function updateChart() {
  ...
  scatter.selectAll("circle")
    ...
    //Solution: Once a graph is made,it is saved to a set of graphs
    //This set can be accessed to update all graphs at once
    //Each graph contains information on its axis, its x/y scale and its scatter point
}
function updateAllGraphs(extent, graphs) {
  if (!extent) {
    if (!idleTimeout)
      return idleTimeout = setTimeout(idled, 350);
      graphs.forEach((graph) => {
        graph.x.domain([0,1]); //TODO Change 0,1 by extremum of x axis
        graph.y.domain([0,1]); //TODO Change 0,1 by extremum of y axis
      });
  } else {
    graphs.forEach((graph) => {
      graph.x.domain([ graph.x.invert(extent[0]), graph.x.invert(extent[1]) ]);
      graph.y.domain([ graph.y.invert(extent[0]), graph.y.invert(extent[1]) ]);
      graph.scatter.select(".brush").call(brush.move, null);
    });
  }
  graphs.forEach((graph) => {
    graph.XAxis.transition().duration(1000).call(d3.axisBottom(graph.x));
    graph.YAxis.transition().duration(1000).call(d3.axisBottom(graph.y));
    graph.scatter.selectAll("circle")
      .transition().duration(1000)
      .attr("cx", function (d) { return x(d.Sepal_Length); } )
      .attr("cy", function (d) { return y(d.Petal_Length); } );
  });
